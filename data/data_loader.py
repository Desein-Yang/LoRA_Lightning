...
class GLUE(Dataset):
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.task_load_path = os.path.join(self.data_load_path,self.task_file_map[self.task_name])
    ...
    def load_data(self):
        print("load data")
        self.data = pd.read_csv(os.path.join(self.task_load_path,self.set_name+'.csv'))
    ...
    def convert_to_features(self,idx):
        ...
        if 'label' in self.data.keys():
            ...
            pdb.set_trace() 
        features = self.tokenizer.encode_plus(sentence,max_length=self.max_seq_length,padding='max_length',truncation=True)
...
class GLUEDataLoader(LightningDataModule):
    ...
    def prepare_data(self):
        task = self.task_name
        self.dataset = {
            "train":GLUE(task_name=task,set_name='train'),
            "test":GLUE(task_name=task,set_name='test'),
            "dev":GLUE(task_name=task,set_name='dev')
        }
    ...
    def train_dataloader(self):
        return DataLoader(self.dataset["train"],batch_size=self.train_batch_size,shuffle=True,collate_fn=collate_fn) 

    def val_dataloader(self):
        """Return validation data"""
        return DataLoader(self.dataset["dev"], batch_size=self.eval_batch_size,collate_fn=collate_fn)
    ...
    def convert_to_features(self,examples):
        ...
        features = self.tokenizer(sentence,max_length=self.max_seq_length,padding=True,truncation=True)
        features['labels'] = label
        return features
...
class GLUEDataModule(LightningDataModule):
    ...
    def from_tsv_csv(self):
        for name in ['train','test','dev']:
            ...
            df.to_csv(self.task_load_path+'/'+name+'.tsv',encoding='utf_8_sig',index=False)
